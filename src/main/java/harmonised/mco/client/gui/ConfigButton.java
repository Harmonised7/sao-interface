...
//                    value = value << 8;
            ...
            case BOOLEAN:
            {
                button1 = new ListButton(box).setButtonColor((boolean) confefeg.get() ? McoConfefeg.buttonActiveColor.get() : McoConfefeg.buttonColor.get()).setMsg(new TranslatableComponent("mco." + confefeg.name)).onPress(theButton ->
                {
                    if((boolean) confefeg.get())
                        confefeg.set(false);
                    else
                        confefeg.set(true);
//                    ((ListButton) theButton).setMsg(new TranslatableComponent("mco." + ((boolean) confefeg.get() ? "on" : "off")));
                    ((ListButton) theButton).setButtonColor((boolean) confefeg.get() ? McoConfefeg.buttonActiveColor.get() : McoConfefeg.buttonColor.get());
                });
                setMsg(new TranslatableComponent(Reference.MOD_ID + "." + confefeg.name));
            }
                break;
        }
    }
