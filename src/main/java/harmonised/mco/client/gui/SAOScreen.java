...
public class SAOScreen extends Screen
{
    ...
    public SAOScreen(Component titleIn)
    {
        super(titleIn);
    }
...
//            SAOScreen.xOffset = 0;
    ...
    private static InfoBox getinfoBox()
    {
        ...
        infoBox.addLine(new TranslatableComponent("mco.healthX", DP.dpSoft(mc.player.getHealth()), DP.dpSoft(mc.player.getMaxHealth())));
        infoBox.addLine(new TranslatableComponent("mco.armorX", DP.dpSoft(mc.player.getTotalArmorValue())));
        infoBox.addLine(new TranslatableComponent("mco.levelX", mc.player.experienceLevel));
        infoBox.addLine(new TranslatableComponent("mco.xpX", DP.dpSoft(mc.player.experience*100f)));
        infoBox.addLine(new TranslatableComponent("mco.hungerX", mc.player.getFoodStats().getFoodLevel(), 20));
        infoBox.addLine(new TranslatableComponent("mco.saturationX", mc.player.getFoodStats().getSaturationLevel(), 20));
    ...
    private static ListBox getPlayerBox()
    {
        ...
        box.addButton(new ListButton(box).setIcon(Icons.SWORD).setMsg(new TranslatableComponent(Reference.MOD_ID + ".equip")).onPress(theButton ->
        {
            openBox((SaoButton) theButton, getEquipTypesBox());
        }));

        box.addButton(new ListButton(box).setIcon(Icons.SWORD).setMsg(new TranslatableComponent(Reference.MOD_ID + ".items")).onPress(theButton ->
        {
        ...
        if(!Util.isReleased())
        {
            box.addButton(new ListButton(box).setIcon(Icons.SWORD).setMsg(new TranslatableComponent(Reference.MOD_ID + ".crafting")).onPress(theButton ->
            {
    ...
    private static ListBox getCraftingTypesBox()
    {
        ...
        box.addButton(new ListButton(box).setItem(Items.COMPASS, false ).setMsg(new TranslatableComponent(Reference.MOD_ID + ".all")).onPress(theButton ->
        {
            openBox((SaoButton) theButton, getCraftingTypeBox(RecipeBookCategories.CRAFTING_SEARCH));
        }));

        box.addButton(new ListButton(box).setItem(Items.REDSTONE, false).setMsg(new TranslatableComponent(Reference.MOD_ID + ".redstone")).onPress(theButton ->
        {
            openBox((SaoButton) theButton, getCraftingTypeBox(RecipeBookCategories.CRAFTING_REDSTONE));
        }));

        box.addButton(new ListButton(box).setItem(Items.BRICKS, false).setMsg(new TranslatableComponent(Reference.MOD_ID + ".blocks")).onPress(theButton ->
        {
            openBox((SaoButton) theButton, getCraftingTypeBox(RecipeBookCategories.CRAFTING_BUILDING_BLOCKS));
        }));

        box.addButton(new ListButton(box).setItem(Items.IRON_AXE, false).setMsg(new TranslatableComponent(Reference.MOD_ID + ".equipment")).onPress(theButton ->
        {
            openBox((SaoButton) theButton, getCraftingTypeBox(RecipeBookCategories.CRAFTING_EQUIPMENT));
        }));

        box.addButton(new ListButton(box).setItem(Items.LAVA_BUCKET, false).setMsg(new TranslatableComponent(Reference.MOD_ID + ".miscellaneous")).onPress(theButton ->
        {
    ...
    private static ListBox updatePartyMembersBox()
    {
        ...
        for(String playerName : PartyPendingSystem.offlineData.keySet())
        {
            ...
            {
                partyMembersBox.addButton(new ListButton(partyMembersBox).setTextColor(0xff00ff).setIcon(Icons.ONE_PERSON).setMsg(new TranslatableComponent("mco.you")).onPress(theButton ->
                {
            ...
            else
            {
                ...
                if(online)
                {
                    double distance = Util.getDistance(mc.player.position(), new Vec3(playerData.getDouble("x"), playerData.getDouble("y"), playerData.getDouble("z")));
    ...
    private static void generatePartyButtons(ListBox box, boolean inParty)
    {
        ...
        if(inParty)
        {
            TranslatableComponent textComp;
            if(PartyPendingSystem.offlineData.size() == 1)
                textComp = new TranslatableComponent("mco.1Member");
            else
                textComp = new TranslatableComponent("mco.xMembers", PartyPendingSystem.offlineData.size());
            ...
            box.addButton(new ListButton(box).setIcon(Icons.MINUS).setMsg(new TranslatableComponent("mco.leave")).onPress(theButton ->
            {
        ...
        {
            box.addButton(new ListButton(box).setIcon(Icons.PLUS).setMsg(new TranslatableComponent("mco.create")).onPress(theButton ->
            {
                mc.getConnection().sendPacket(new CChatMessagePacket("/pmmo party create"));
                generatePartyButtons(box, inParty());
            }));

            box.addButton(new ListButton(box).setIcon(Icons.CHECKMARK).setMsg(new TranslatableComponent("mco.accept")).onPress(theButton ->
            {
            ...
            box.addButton(new ListButton(box).setIcon(Icons.X).setMsg(new TranslatableComponent("mco.decline")).onPress(theButton ->
            {
    ...
    private static ListBox getPmmoSkillsBox(UUID uuid)
    {
        ...
        SaoButton totalLevelButton = new ListButton(box).setIcon(Icons.STATS).setMsg(new TranslatableComponent("pmmo.levelDisplay", DP.dpSoft(XP.getTotalLevelFromMap(xpMap)), new TranslatableComponent("pmmo.totalLevel"))).onPress(theButton ->
        {
            openBox((SaoButton) theButton, getPmmoHiscoreBox("totalLevel"));
        });
        ...
        {
            SaoButton button = new ListButton(box).setIcon(Icons.STATS).setMsg(new TranslatableComponent("pmmo.levelDisplay", DP.dpSoft(XP.levelAtXpDecimal(skill.getValue())), new TranslatableComponent("pmmo." + skill.getKey()))).onPress(theButton ->
            {
    ...
    private static ListBox getMenuBox()
    {
        ...
        box.addButton(new ListButton(box).setIcon(Icons.GEAR).setMsg(new TranslatableComponent(Reference.MOD_ID + ".settings")).onPress(theButton ->
        {
            openBox((SaoButton) theButton, getSettingsBox());
        }));

        box.addButton(new ListButton(box).setLock(Math.random() <= 0.01).setIcon(Icons.SWORD).setMsg(new TranslatableComponent(Reference.MOD_ID + ".logout")).onPress(theButton ->
        {
    ...
    private static ListBox getSettingsBox()
    {
        ...
        box.addButton(new ListButton(box).setIcon(Icons.GEAR).setMsg(new TranslatableComponent(Reference.MOD_ID + ".reloadAll")).onPress(theButton ->
        {
        ...
        {
            box.addButton(new ListButton(box).setIcon(Icons.GEAR).setMsg(new TranslatableComponent("confefeg." + confefeger.confefegName)).onPress(theButton ->
            {
    ...
    private static ListBox getConfefegerBox(Confefeger confefeger)
    {
        ...
        box.addButton(new ListButton(box).setIcon(Icons.GEAR).setMsg(new TranslatableComponent(Reference.MOD_ID + ".reload")).onPress(theButton ->
        {
        ...
        {
            box.addButton(new ListButton(box).setIcon(Icons.GEAR).setMsg(new TranslatableComponent(Reference.MOD_ID + "." + entry.getKey())).onPress(theButton ->
            {
    ...
    private static ListBox getConfefegsBox(String boxKey, Set<Confefeger.Confefeg> confefegs)
    {
        ...
        {
            configButtons.add(new ConfigButton(box, confefeg).setIcon(Icons.GEAR).setMsg(new TranslatableComponent(Reference.MOD_ID + "." + confefeg.name)).onPress(theButton ->
            {
            }));
        }
    ...
    private static ListBox getEquipTypesBox()
    {
        ...
        box.addButton(new ListButton(box).setIcon(Icons.SWORD).setMsg(new TranslatableComponent(Reference.MOD_ID + ".armor")).onPress(theButton ->
        {
            openBox((SaoButton) theButton, getEquipArmorBox());
        }));

        box.addButton(new ListButton(box).setIcon(Icons.SWORD).setMsg(new TranslatableComponent(Reference.MOD_ID + ".weapons")).onPress(theButton ->
        {
    ...
    private static ListBox getSelectWeaponBox()
    {
        ...
        box.addButton(new ListButton(box).setItem(Items.DIAMOND_SWORD, false).setMsg(new TranslatableComponent(Reference.MOD_ID + ".swords")).onPress(theButton ->
        {
            openBox((SaoButton) theButton, getSwapSlotTypeBox(((TranslatableComponent)theButton.getMessage()).getKey().substring(Reference.MOD_ID.length() + 1)));
        }));

        box.addButton(new ListButton(box).setItem(Items.DIAMOND_AXE, false).setMsg(new TranslatableComponent(Reference.MOD_ID + ".axes")).onPress(theButton ->
        {
            openBox((SaoButton) theButton, getSwapSlotTypeBox(((TranslatableComponent)theButton.getMessage()).getKey().substring(Reference.MOD_ID.length() + 1)));
        }));

        box.addButton(new ListButton(box).setItem(Items.BOW, false).setMsg(new TranslatableComponent(Reference.MOD_ID + ".bows")).onPress(theButton ->
        {
            openBox((SaoButton) theButton, getSwapSlotTypeBox(((TranslatableComponent)theButton.getMessage()).getKey().substring(Reference.MOD_ID.length() + 1)));
        }));

        box.addButton(new ListButton(box).setItem(Items.CROSSBOW, false).setMsg(new TranslatableComponent(Reference.MOD_ID + ".crossbows")).onPress(theButton ->
        {
            openBox((SaoButton) theButton, getSwapSlotTypeBox(((TranslatableComponent)theButton.getMessage()).getKey().substring(Reference.MOD_ID.length() + 1)));
        }));
    ...
    private static ListBox getSelectItemsBox()
    {
        ...
        box.addButton(new ListButton(box).setItem(Items.GOLDEN_AXE, false).setMsg(new TranslatableComponent(Reference.MOD_ID + ".tools")).onPress(theButton ->
        {
            openBox((SaoButton) theButton, getSelectToolsBox());
        }));

        box.addButton(new ListButton(box).setItem(Items.FLINT_AND_STEEL, false).setMsg(new TranslatableComponent(Reference.MOD_ID + ".items")).onPress(theButton ->
        {
            openBox((SaoButton) theButton, getSwapSlotTypeBox(((TranslatableComponent)theButton.getMessage()).getKey().substring(Reference.MOD_ID.length() + 1)));
        }));

        box.addButton(new ListButton(box).setItem(Items.STONE_BRICKS, false).setMsg(new TranslatableComponent(Reference.MOD_ID + ".blocks")).onPress(theButton ->
        {
            openBox((SaoButton) theButton, getSwapSlotTypeBox(((TranslatableComponent)theButton.getMessage()).getKey().substring(Reference.MOD_ID.length() + 1)));
        }));

        box.addButton(new ListButton(box).setItem(Items.GOLDEN_CARROT, false).setMsg(new TranslatableComponent(Reference.MOD_ID + ".food")).onPress(theButton ->
        {
            openBox((SaoButton) theButton, getSwapSlotTypeBox(((TranslatableComponent)theButton.getMessage()).getKey().substring(Reference.MOD_ID.length() + 1)));
        }));

        box.addButton(new ListButton(box).setItem(Items.REDSTONE_TORCH, false).setMsg(new TranslatableComponent(Reference.MOD_ID + ".redstone")).onPress(theButton ->
        {
            openBox((SaoButton) theButton, getSwapSlotTypeBox(((TranslatableComponent)theButton.getMessage()).getKey().substring(Reference.MOD_ID.length() + 1)));
        }));

        box.addButton(new ListButton(box).setItem(Items.POTION, false).setMsg(new TranslatableComponent(Reference.MOD_ID + ".potions")).onPress(theButton ->
        {
            openBox((SaoButton) theButton, getSwapSlotTypeBox(((TranslatableComponent)theButton.getMessage()).getKey().substring(Reference.MOD_ID.length() + 1)));
        }));
    ...
    private static ListBox getSelectToolsBox()
    {
        ...
        box.addButton(new ListButton(box).setItem(Items.GOLDEN_PICKAXE, false).setMsg(new TranslatableComponent(Reference.MOD_ID + ".pickaxes")).onPress(theButton ->
        {
            openBox((SaoButton) theButton, getSwapSlotTypeBox(((TranslatableComponent)theButton.getMessage()).getKey().substring(Reference.MOD_ID.length() + 1)));
        }));

        box.addButton(new ListButton(box).setItem(Items.GOLDEN_AXE, false).setMsg(new TranslatableComponent(Reference.MOD_ID + ".axes")).onPress(theButton ->
        {
            openBox((SaoButton) theButton, getSwapSlotTypeBox(((TranslatableComponent)theButton.getMessage()).getKey().substring(Reference.MOD_ID.length() + 1)));
        }));

        box.addButton(new ListButton(box).setItem(Items.GOLDEN_SHOVEL, false).setMsg(new TranslatableComponent(Reference.MOD_ID + ".shovels")).onPress(theButton ->
        {
            openBox((SaoButton) theButton, getSwapSlotTypeBox(((TranslatableComponent)theButton.getMessage()).getKey().substring(Reference.MOD_ID.length() + 1)));
        }));

        box.addButton(new ListButton(box).setItem(Items.GOLDEN_HOE, false).setMsg(new TranslatableComponent(Reference.MOD_ID + ".hoes")).onPress(theButton ->
        {
            openBox((SaoButton) theButton, getSwapSlotTypeBox(((TranslatableComponent)theButton.getMessage()).getKey().substring(Reference.MOD_ID.length() + 1)));
        }));
...
//                }
    ...
    private static ListBox getEquipArmorBox()
    {
        ...
        box.addButton(new ListButton(box).setItem(Items.DIAMOND_HELMET, false).setMsg(new TranslatableComponent(Reference.MOD_ID + ".head")).onPress(theButton ->
        {
            openBox((SaoButton) theButton, getEquipSlotTypeBox(EquipmentSlotType.HEAD));
        }));
        box.addButton(new ListButton(box).setItem(Items.DIAMOND_CHESTPLATE, false).setMsg(new TranslatableComponent(Reference.MOD_ID + ".chest")).onPress(theButton ->
        {
            openBox((SaoButton) theButton, getEquipSlotTypeBox(EquipmentSlotType.CHEST));
        }));
        box.addButton(new ListButton(box).setItem(Items.DIAMOND_LEGGINGS, false).setMsg(new TranslatableComponent(Reference.MOD_ID + ".legs")).onPress(theButton ->
        {
            openBox((SaoButton) theButton, getEquipSlotTypeBox(EquipmentSlotType.LEGS));
        }));
        box.addButton(new ListButton(box).setItem(Items.DIAMOND_BOOTS, false).setMsg(new TranslatableComponent(Reference.MOD_ID + ".feet")).onPress(theButton ->
        {
            openBox((SaoButton) theButton, getEquipSlotTypeBox(EquipmentSlotType.FEET));
        }));
        box.addButton(new ListButton(box).setItem(Items.SHIELD, false).setMsg(new TranslatableComponent(Reference.MOD_ID + ".shields")).onPress(theButton ->
        {
            openBox((SaoButton) theButton, getSwapSlotTypeBox(((TranslatableComponent)theButton.getMessage()).getKey().substring(Reference.MOD_ID.length() + 1)));
        }));
